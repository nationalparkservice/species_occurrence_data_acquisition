...
# Note: Try commenting out this first section (down to the next comment)if you 
#     are getting errors when trying to connect to the duckdb.
...
# 
# sppList_birds[is.na(sppList_birds$sort2022_eBird), c(1:3, 13, 11:12, 23:24)] |> 
#   View()
...
ITIS ID's were assigned to the *CleanSourceSci* field and used to match with the IDIS ID from the clean species names created from the GBIF data because the species names fields from the conservation lists and the GBIF data don't match neatly.

```{r conservLists, eval=TRUE}
...
**TODO:** Evaluate the rest of the script and make sure it runs after the *eval* options have been included in the chunk headers.
...
```{r sppConcern, eval=FALSE, results='hold'}
...
lapply(spp_status, function(status){
  tmp_df <- spp_df[spp_df$fedStatus == status, ] |>
    dplyr::arrange(kingdom_gbif, class_gbif, sort2022_eBird, 
                       scientificName_gbif) |> 
...
```{r occMap, eval=FALSE, results='hold'}
