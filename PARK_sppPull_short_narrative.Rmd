---
title: "External Species Occurrence Data for Cane River Creole National Historical Park"
author:
  - name: "Tom Philippi"
...
params:
  UnitCode: "CARI"
  UnitName: "Cane River Creole National Historical Park"
...
This summary of external species occurrence data provides park planners and resource managers at `r params$UnitName` existing species occurrence records from GBIF- The Global Biodiversity Information Facility in and near their park, guidance in use and interpretation of these data, and full provenance of the downloaded dataset so anomalous observations may be tracked all the way back to the source for verification or flagging. 

What’s Included:\

* This report in HTML format documenting the data acquisition, simple guidance for use of the data and the species data tables in delimited format and [Darwin Core Archive format](https://www.gbif.org/darwin-core) (GBIF, 2022a). 

Many National Park resource management issues require information about species occurrences to inform planning and management actions. The [GBIF repository](https://www.gbif.org/) (2022b) is a rapidly growing set of nearly one billion externally-sourced species occurrence records and can be leveraged to answer some species occurrence questions. In some cases, these existing externally-sourced records can be used to scope and target planned field surveys to address specific issues and can fill in gaps in our knowledge of species occurrences in parks. 

These are presence-only records and can document the presence of a species in an area of interest and identify species “probably” or “possibly present.” It is important to understand the limitations and known issues of these data.
...
The source dataset is updated at best monthly and occasionally less frequently. Hence, the technique used to generate these data is repeatable which is why the code file generates the report. This approach provides three tiers of species occurrence data: 1) species that have been recorded in the park, 2) species that likely occur in the park but have not been recorded in the park, and 3) species that occur near the park that have not been recorded in the park. 

Existing species occurrences are summarized for three spatial areas of analysis for `r params$UnitName` (Figure 1). This approach provides three tiers of species occurrence data: 

  * species that have been recorded in the park,
  * species that likely occur in the park but have not been recorded in the park, and
...
#   htmltools::HTML(
#     sprintf("Figure 1. %s Areas of Analysis - %s", 
#             params$UnitName, 
...
if(params$Key == "new"){
  ...
  gbifDwnld <- rgbif::occ_download(gbifPred, user = GBusername,
                             pwd = GBpwd,
                             email = GBemail,
                             format = "DWCA")
...
The data downloaded are spatially `r ifelse(isTRUE(validGBIF), "valid", "not valid")`. There are `r localeTbl[localeTbl$Var1 == 'park', 2]` external species occurrence records within the park boundary, another `r localeTbl[localeTbl$Var1 == 'km3', 2]` records within 3km of the park boundary, and `r localeTbl[localeTbl$Var1 == 'km30', 2]` more records between 3km and 30km of the boundary (Table 2).
Records were obtained from `r nrow(typesTable)` collections or observational GBIF record types (Table 3)
and from `r nrow(instituteTable)` sources (e.g., museums or data repositories) (Table 4).

Table 5 displays record counts in the park boundary by taxonomic class and table 6 summarizes species counts in the park boundary by taxonomic class.
...
Taxonomic names from ITIS are accessed on `r lubridate::date(file.info(file.path("..", "Taxa Lists", "duckdb"))$ctime)` using the taxadb package. The eBird checklist was downloaded from the Cornell Lab of Ornithology on `r lubridate::date(file.info(file.path(eBirdChecklistFile))$ctime)`.
...
For all records inside and within 30km of `r params$UnitName`, non-matching scientific names are noted in Table 7. Table 8 shows counts of missing scientific names from either source while Table 9 counts mismatched taxonomic classes between ITIS and GBIF. If non-matches and/or missing records occur for eBird information, summaries are noted in Tables 10-12.
...
# taxadb::td_create(provider = "itis", schema = "dwc", overwrite = FALSE, 
#                   dbdir = here::here())
# taxadb::td_connect(here::here())
...
nonMatchingScientificNames <- sppList |>
  dplyr::filter(scientificName_gbif != scientificName_itis) |>
  dplyr::distinct() |> 
  dplyr::group_by(kingdom_gbif, kingdom_itis, phylum_gbif, phylum_itis) |>
  dplyr::summarize(n = n()) |> 
  dplyr::rename("Count of Non-Matching Scientific Names" = n)

missingSciNamesTable <- sppList |> 
  dplyr::filter(is.na(scientificName_gbif) | is.na(scientificName_itis)) |>
  dplyr::distinct() |> 
  dplyr::group_by(kingdom_gbif, kingdom_itis, phylum_gbif, phylum_itis) |> 
  dplyr::summarize(n = n()) |>
  dplyr::rename("Count of Missing Scientific Names" = n)

nonMatchingClassesTable <- sppList |>
  dplyr::filter(class_gbif != class_itis) |>
  dplyr::distinct() |> 
  dplyr::group_by(kingdom_gbif, kingdom_itis, phylum_gbif, phylum_itis) |> 
  dplyr::count(class_gbif, class_itis, name = 'Count of Non-matching Class Values')

myTable(nonMatchingScientificNames, 
        col_names = c("Kingdom GBIF", "Kingdom ITIS", "Phylum GBIF",  "Phylum ITIS", "Count"),
        caption = sprintf("Table 7. Non-matching scientific name values for records observed (of %s records)", nrow(sppList)))

myTable(missingSciNamesTable, 
        col_names = c("Kingdom GBIF", "Kingdom ITIS", "Phylum GBIF",  "Phylum ITIS", "Count"),
        caption = sprintf("Table 8. Number of missing scientific names for records observed  (of %s records)", nrow(sppList)))

myTable(nonMatchingClassesTable, 
        col_names = c("Kingdom GBIF", "Kingdom ITIS", "Phylum GBIF",  "Phylum ITIS", "Class GBIF", "Class ITIS", "Count"),
        caption = sprintf("Table 9. Number of non-matching class values for records observed (of %s records)", nrow(sppList)))

#sppList[sppList$taxonRank == "genus" | is.na(sppList$taxonRank),
#        c(1:3, 13, 11:12, 23)] |>
#  View()
# Case does not match for these columns:
...
#birds <- birds |> dplyr::rename(phylum_itis = plylum_itis)
...
# 
# Show records without eBird matches:
...
nonMatchingEBirdTable <- sppList_birds[is.na(sppList_birds$sort2021_eBird), c(1:3, 13, 11:12, 23:24)]

nonMatchingScientificNamesEBirdTable <- sppList_birds |>
  dplyr::filter(scientificName_gbif != scientificName_itis) |>
  dplyr::distinct() |> 
  dplyr::group_by(kingdom_gbif, kingdom_itis, phylum_gbif, phylum_itis) 

missingSciNamesEBirdTable <- sppList_birds |> 
  dplyr::filter(is.na(scientificName_gbif) | is.na(scientificName_itis)) |>
  dplyr::distinct() |> 
  dplyr::group_by(kingdom_gbif, kingdom_itis, phylum_gbif, phylum_itis) |> 
  dplyr::count(scientificName_gbif, scientificName_itis, name = 'Count')

nonMatchingClassesEBirdTable <- sppList_birds |>
  dplyr::filter(class_gbif != class_itis) |>
  dplyr::distinct() |> 
  dplyr::group_by(kingdom_gbif, kingdom_itis, phylum_gbif, phylum_itis) |> 
  dplyr::count(class_gbif, class_itis, name = 'Count')
...
if(nrow(nonMatchingEBirdTable) > 0) {
myTable(nonMatchingEBirdTable, 
        col_names = c("itisID", "taxonKey_gbif", "taxonRank_gbif", "taxonRank_itis", "scientificName_gbif", "scientificName_itis", "vernacularName", "sort2021_eBird"),
        caption = "Table 10. Non-matching eBird records")
}

myTable(missingSciNamesEBirdTable, 
          col_names = c("Kingdom GBIF", "Kingdom ITIS", "Phylum GBIF",  "Phylum ITIS", "Scientific Name GBIF", "Scientific Name ITIS", "Count"),
          caption = "Table 11. Non-matching scientific name values (GBIF and ITIS vs. eBird)")


myTable(nonMatchingClassesEBirdTable, 
        col_names = c("Kingdom GBIF", "Kingdom ITIS", "Phylum GBIF",  "Phylum ITIS", "Class GBIF", "Class ITIS", "Count"),
        caption = "Table 12. Non-matching scientific name values (GBIF and ITIS vs. eBird)")
...
sppList <- merge(sppList, localeCount,
  ...
  dplyr::arrange(kingdom_gbif, phylum_gbif, class_gbif, 
                 ...
                 scientificName_gbif) #|> 
  # dplyr::rename(occurrencesInPark = occurrencesInPark.y,
  #               occurrencesIn3km = occurrencesIn3km.y,
  #               occurrencesIn30km = occurrencesIn30km.y,
  #               totalOccurrences = totalOccurrences.y
  #               )
...
See the `r paste0(params$UnitCode, "_SpeciesTable.csv")` delimited output table for the externally-sourced species occurrence data for `r params$UnitName`. Column definitions are noted in Table 13. 

Table 13. Column Definitions in Delimited Output Table

|   Column Name   |   Column Definition
|   :-----------    |   :-----------------
|   taxonKey_gbif   |   taxonomy key used in GBIF
|   scientificName_gbif   |   scientific name used in GBIF
|   itisID   |   idenfifier used in ITIS
|   scientificName_itis   |   scientific name used in ITIS
|   vernacularName   |   vernacular name in GBIF or eBird
|   taxonRank_gbif   |   taxon rank used in GBIF
|   kingdom_gbif   |   taxonomic kingdom used in GBIF
|   phylum_gbif    |   taxonomic phylum used in GBIF
|   class_gbif   |   taxonomic class used in GBIF
|   order_gbif   |   taxonomic order used in GBIF
|   family_gbif   |   taxonomic family used in GBIF
|   genus_gbif   |   taxonomic genus used in GBIF
|   sort2021_eBird   |   eBird sort value
...
This report uses R (version `r paste(sessionInfo()$R.version$major, sessionInfo()$R.version$minor, sep=".")`, R Core Team 2022), the rgbif package (version `r sessionInfo()$otherPkgs$rgbif$Version`, Chamberlain, S. et. al., 2022) and the taxadb package (version `r sessionInfo()$otherPkgs$taxadb$Version`, Boettinger, C. et. al., 2022) to access data from the Global Biodiversity Information Facility (GBIF) resource (Waller,  2021). 

# References

Boettinger, C., Norman, K., Poelen, J., Chamberlain, S. and N. Ross. 2022. TaxaDB - A High-Performance Local Taxonomic Database Interface. <https://github.com/ropensci/taxadb>
...
Global Biodiversity Information Facility. 2022a. What is Darwin Core, and why does it matter?. <https://www.gbif.org/darwin-core>.

Global Biodiversity Information Facility. 2022b. GBIF: The Global Biodiversity Information Facility (2022) What is GBIF?. <https://www.gbif.org/what-is-gbif>.
...
Kari E. A. Norman, Scott Chamberlain, and Carl Boettiger. 2020.
  ...
  <https://doi:10.1111/2041-210X.13440>.
